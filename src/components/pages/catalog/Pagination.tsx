...
const Pagination: React.FC<{ query: React.MutableRefObject<string> }> = ({ query }) => {
  ...
  let currentChunk = $productsStore.currentChunk

  const ITEMS_PER_PAGE = 10
  const pagesCount = Math.ceil($productsStore.total / ITEMS_PER_PAGE)
  ...
  const onPrevPage = () => {
    if (currentChunk > 0) {
      ...
      handleFetch(handleQuery(--currentChunk))
    }
  }
  ...
  const onPageChange = (num: number) => handleFetch(handleQuery(num))
